    ...
    var message: String {
        ...
        case .dictationWords:
            if !showGitHubStar && !showBuyMeCoffee && (showTweetAboutMuesli || showPostOnLinkedIn) {
                return "That is a serious pile of words. If \(AppIdentity.displayName) has been saving your fingers and your flow, sharing your milestone helps more people find it."
            }
            return "That is a serious pile of words. If \(AppIdentity.displayName) has been saving your fingers and your flow, a GitHub star or a coffee helps keep it moving."
        case .meetings:
            return "That is a lot of conversations turned into something useful. If \(AppIdentity.displayName) has been keeping your meetings in order, a GitHub star or a coffee helps keep it moving."
        }
    }
}
...
enum ContributionSocialShare {
    ...
    static func message(wordCount: Int) -> String {
        "I've dictated \(formatCount(wordCount)) words with \(AppIdentity.displayName). It's fast, open source, on-device, and free to use. Try it: \(muesliURL.absoluteString)"
    }
...
enum ContributionMilestonePolicy {
    ...
    static func prompt(
        ...
        // Disabled for this local build — no star / coffee / social nags.
        return nil
    }
}

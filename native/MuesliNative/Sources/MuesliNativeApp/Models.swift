...
struct PostProcessorOption: Identifiable, Equatable {
    ...
    static let finetunedV2 = PostProcessorOption(
        ...
        sizeLabel: "~390 MB",
        description: "Fine-tuned on Pip dictation data. Best for filler removal, deletion cues, and spoken list formatting.",
    ...
    static let finetunedV3 = PostProcessorOption(
        ...
        sizeLabel: "~505 MB",
        description: "Fine-tuned Qwen3.5-0.8B on Pip dictation data. Improved over v2 on filler removal, deletion cues, and spoken list formatting.",
...
struct HotkeyConfig: Codable, Equatable {
    ...
    /// Keys allowed in a modifier+key combination (letters + Space).
    static func combinationKeyLabel(for keyCode: UInt16) -> String? {
        if keyCode == 49 { return "Space" }
        return letterLabel(for: keyCode)
    }
    ...
    /// Whisper-style: fn+Space starts hands-free dictation; fn alone stops and pastes.
    static let fnSpace = HotkeyConfig.combination(modifiers: [.function], keyCode: 49)

    static func isWhisperStyle(_ config: HotkeyConfig) -> Bool {
        if config.keyCode == 63 && !config.isCombination { return true }
        return config.combinationKeyCode == 49
            && config.resolvedCombinationModifiers == .function
    }
...
struct AppConfig: Codable {
    ...
    var customWords: [CustomWord] = [
        CustomWord(word: "pip", replacement: "Pip"),
    ]

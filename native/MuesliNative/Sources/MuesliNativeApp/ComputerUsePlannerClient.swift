...
enum ComputerUsePlannerClient {
    ...
    static var instructions: String {
        """
    You are Pip's computer-use planner. You do not execute actions. You must choose exactly one native tool call from the provided tool list.
    ...
    - After launch_app, Pip will refresh the requested app's state automatically. If the next state is not the requested app, call get_app_state for that app before using fail.
    ...
    - type_text sends literal keyboard input after Pip activates the requested app and verifies a focused editable target. Use it for normal typing into focused text fields.
    ...
    - Risky actions are locally blocked by Pip; do not try to bypass confirmation.
    """
    }

...
struct SettingsView: View {
    ...
    private var syncSettingsPane: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing24) {
            settingsSection("iCloud Text Sync") {
                ...
                settingsDescription("Sync dictation text, meeting transcripts, notes, summaries, and manual notes with \(AppIdentity.displayName) for iPhone through your private iCloud account. Audio recordings are never synced.")
            ...
            settingsSection("iPhone Bridge") {
                ...
                settingsDescription("Keep the timeline bridge card available while users connect \(AppIdentity.displayName) on iPhone.")

                Divider().background(MuesliTheme.surfaceBorder)

                HStack(spacing: MuesliTheme.spacing12) {
                    VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                        Text("\(AppIdentity.displayName) for iPhone")
                            .font(MuesliTheme.body())
                            .foregroundStyle(MuesliTheme.textPrimary)
    ...
    private var syncStatusText: String {
        if !appState.config.iCloudSyncEnabled {
            return "Sync is off. Turn it on to bridge this Mac with \(AppIdentity.displayName) for iPhone."
        }
    ...
    private func pickCustomAudioFile() {
        ...
        do {
            let supportDir = appSupportBase
                .appendingPathComponent(Bundle.main.infoDictionary?["MuesliSupportDirectoryName"] as? String ?? "Pip")

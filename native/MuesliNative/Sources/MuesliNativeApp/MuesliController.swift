...
enum MeetingRetranscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .controllerUnavailable:
            return "Meeting re-transcription could not continue because \(AppIdentity.displayName) is no longer available."
        case .recordingUnavailable:
...
final class MuesliController: NSObject {
    ...
    private var didProvideDictationStartFeedback = false
    ...
    private func refreshContributionMilestonePrompt(totalWords: Int, totalMeetings: Int) {
        // Local build: disable milestone nag prompts (star / coffee / social share).
        _ = totalWords
        _ = totalMeetings
    ...
    /// Fire listening beep + recording UI as soon as the hotkey commits,
    /// without waiting for the first mic buffer (streamActive). Capture often
    /// starts earlier than UI feedback; delaying the cue feels laggy.
    private func provideImmediateDictationStartFeedback() {
        guard !didProvideDictationStartFeedback else { return }
        didProvideDictationStartFeedback = true
        activateDictationRecordingIndicator()
        markDictationLatency("sound_start_requested:immediate")
        SoundController.playDictationStart(enabled: shouldPlayDictationLifecycleSounds && !isDictationTestMode)
    }

    private func resetDictationStartFeedback() {
        didProvideDictationStartFeedback = false
    }
    ...
    private func handleDictationStreamActive(capturedAt: Date) {
        ...
        if didProvideDictationStartFeedback {
            // Cue already played on hotkey; just keep the power meter wired.
            if !isDictationTestMode {
                indicator.powerProvider = { [weak self] in
                    self?.dictationAudioSessionManager.currentPower() ?? -160
                }
    ...
    private func handleStart() {
        ...
        provideImmediateDictationStartFeedback()

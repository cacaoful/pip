...
struct ShortcutsView: View {
    ...
    private var dictationShortcutSection: some View {
        VStack(alignment: .leading, spacing: MuesliTheme.spacing16) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: MuesliTheme.spacing4) {
                    Text("Push to Talk")
                        ...
                        .foregroundStyle(MuesliTheme.textPrimary)
                    Text(dictationShortcutHelpText)
            ...
            Button {
                applyWhisperStyleShortcut()
            } label: {
                Text("Use fn+Space (Whisper-style)")
                    .font(MuesliTheme.body())
                    .foregroundStyle(MuesliTheme.textPrimary)
            }
            .buttonStyle(.plain)
            ...
            .disabled(appState.config.dictationHotkey == .fnSpace
                && appState.config.hotkeyTriggerThresholdMS == HotkeyTriggerTiming.minThresholdMilliseconds)
            .help("Hold fn to talk; fn+Space for hands-free; fn again to paste. Disable macOS “Select next source in Input menu” if fn+Space is stolen.")
    ...
    private var dictationShortcutHelpText: String {
        if HotkeyConfig.isWhisperStyle(appState.config.dictationHotkey) {
            return "Hold fn to talk; fn+Space locks hands-free; press fn again to paste"
        }
        if appState.config.dictationHotkey.isCombination {
            return "Press \(appState.config.dictationHotkey.displayLabel) to toggle hands-free dictation"
        }
        return "Hold to record, release to transcribe"
    }

    private func applyWhisperStyleShortcut() {
        let result = controller.updateDictationHotkey(.fnSpace)
        controller.updateConfig { $0.hotkeyTriggerThresholdMS = HotkeyTriggerTiming.minThresholdMilliseconds }
        if result.didUpdate {
            dictationShortcutMessage = "Hold fn to talk, or fn+Space for hands-free, then fn to paste. If fn+Space is stolen, disable macOS Input Sources → “Select next source in Input menu”."
            computerUseShortcutMessage = nil
            meetingRecordingShortcutMessage = nil
        } else {
            dictationShortcutMessage = result.message
        }
    }
    ...
    private func recordingPrompt(for target: ShortcutTarget) -> String {
        ...
        case .dictation:
            return "Press modifier or fn+Space..."
        case .computerUse:
    ...
    private func startRecording(_ target: ShortcutTarget) {
        ...
        eventMonitor = NSEvent.addLocalMonitorForEvents(matching: [.flagsChanged, .keyDown]) { [self] event in
            if event.type == .keyDown {
                ...
                let hasModifiers = !mods.isEmpty
                let allowsCombination = target == .meetingRecording || target == .dictation

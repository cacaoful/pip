...
struct TranscribeCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Transcribe a local audio file with Pip's bundled Parakeet models."
    )
...
struct MuesliAudioTranscriptionPipeline {
    ...
    static func rawTranscriptNotes(transcript: String, title: String, summaryRequested: Bool, warnings: [String]) -> String {
        ...
        if summaryRequested {
            ...
            if warnings.isEmpty {
                sections.append("Pip could not generate structured notes from the configured summary backend.")
            } else {
...
enum CLISummaryClient {
    ...
    static func summarize(transcript: String, title: String, config: CLISummaryConfig) async throws -> String {
        ...
        default:
            throw CLISummaryError.unavailable("The configured ChatGPT session summary backend is app-only in headless CLI mode. Select OpenAI, OpenRouter, Ollama, LM Studio, or Custom LLM in Pip settings for `muesli-cli transcribe --summarize`.")
        }
    }

...
struct BackendOptionTests {
    ...
    func knownBackends() {
        let known: Set<String> = ["fluidaudio", "whisper", "qwen", "nemotron35", "cohere", "indicasr", "sensevoice"]
        for option in BackendOption.all {
...
struct HotkeyMonitorTests {
    ...
    @Test("fn+Space preset configures whisper-style hybrid mode")
    @MainActor
    func fnSpacePresetConfiguresWhisperStyleHybridMode() {
        let scheduler = ManualHotkeyScheduler()
        let monitor = scheduler.makeMonitor(startDelay: 0.03)
        monitor.configure(.fnSpace)
        #expect(monitor.whisperStyleEnabled)
        #expect(monitor.targetKeyCode == 63)
        #expect(!monitor.isCombinationMode)
    }

    @Test("fn alone stops active combination toggle")
    @MainActor
    func fnAloneStopsActiveCombinationToggle() {
        let scheduler = ManualHotkeyScheduler()
        let monitor = scheduler.makeMonitor(startDelay: 0.03)
        // Non-whisper combination still supports fn-as-stop while toggle is active.
        monitor.configure(HotkeyConfig.combination(modifiers: [.command, .shift], keyCode: 15))
        var toggleStartCount = 0
        var toggleStopCount = 0
        monitor.onToggleStart = {
        ...
        monitor.handleCombinationForTests(type: .flagsChanged, keyCode: 63, flags: .function)
    ...
    @Test("whisper style hold fn starts push to talk without double-tap delay")
    @MainActor
    func whisperStyleHoldFnStartsPushToTalkWithoutDoubleTapDelay() {
        let scheduler = ManualHotkeyScheduler()
    ...
    @Test("whisper style fn plus space locks hands free and fn stops")
    @MainActor
    func whisperStyleFnPlusSpaceLocksHandsFreeAndFnStops() {
        let scheduler = ManualHotkeyScheduler()
        let monitor = scheduler.makeMonitor(prepareDelay: 0, startDelay: 0.05)
        monitor.configure(.fnSpace)
        #expect(monitor.whisperStyleEnabled)
        ...
        monitor.handleFlagsChanged(keyCode: 63, flags: .function)
        let consumed = monitor.handleKeyDown(keyCode: 49, flags: .function) // Space
        #expect(consumed)
        #expect(toggleStartCount == 1)
        #expect(monitor.isToggleRecording)

        // Releasing fn must not stop hands-free.
...
struct HotkeyConfigTests {
    ...
    @Test("fn+Space whisper-style preset label")
    func fnSpaceWhisperStylePresetLabel() {
        #expect(HotkeyConfig.fnSpace.isCombination)
        #expect(HotkeyConfig.fnSpace.combinationKeyCode == 49)
        #expect(HotkeyConfig.fnSpace.resolvedCombinationModifiers == .function)
        #expect(HotkeyConfig.fnSpace.label == "fn+Space")

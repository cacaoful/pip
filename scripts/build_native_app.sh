...
if [[ "$SKIP_SIGN" != "1" ]]; then
  if ! security find-identity -v -p codesigning | grep -Fq "$SIGN_IDENTITY"; then
    ...
    echo "For local Pip builds without this certificate, run: ./scripts/dev-test.sh (auto-sets MUESLI_SKIP_SIGN=1)" >&2
    exit 1
  fi

...
#
# Default behavior:
# - Resets all permissions for com.pip.app using tccutil
# - Refuses to run while Pip appears to be running unless forced
# - Prints the manual next steps required to re-grant permissions
#
# Usage:
#   ./scripts/dev-reset-permissions.sh
#   ./scripts/dev-reset-permissions.sh --bundle-id com.pip.app
#   ./scripts/dev-reset-permissions.sh --dry-run

BUNDLE_ID="${MUESLI_DEV_BUNDLE_ID:-com.pip.app}"
APP_PROCESS_NAME="${MUESLI_DEV_PROCESS_NAME:-Pip}"
APP_BUNDLE_PATH="${MUESLI_DEV_APP_PATH:-/Applications/Pip.app}"
DRY_RUN=0
FORCE=0
